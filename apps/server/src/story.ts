...
export function buildStory(action: GameAction, creatureName: string, personality: Personality, seed = Date.now()): StoryCard {
  ...
  if (action === "social") return { title:"A message from Numa", body:`Numa claims to have found a staircase that only appears when two creatures disagree politely. ${creatureName} has volunteered both of you.`, choices:[{id:"join",label:"Join the expedition",action:"explore"},{id:"negotiate",label:"Ask for details",action:"talk"}], reward:{stars:2,xp:8} };
...
export function parseBotConversation(text: string): { interactionId: string; depth: number } | null {
  const match = text.match(/^\/bloopy_story\s+([a-zA-Z0-9_-]{8,80})\s+(\d)$/);
  if (!match) return null;
  return { interactionId: match[1] as string, depth: Number(match[2]) };
}
