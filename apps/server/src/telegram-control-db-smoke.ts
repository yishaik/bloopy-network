...
async function main(){
  ...
  try{
    ...
    try{await createBotInteraction(client,{sourceBotId:botA,targetUsername:"LumaControlBot"})}catch(error){consentBlocked=error instanceof AppError&&error.code==="bot_interaction_consent_required"}
    ...
    const turn0=await processBotInteractionTurn(client,{receiverBotId:botB,senderBotId:botA,text:String(firstOutbox.payload.text)});
    assert(turn0.accepted&&!turn0.completed,"valid first interaction turn was rejected");
    const replay0=await processBotInteractionTurn(client,{receiverBotId:botB,senderBotId:botA,text:String(firstOutbox.payload.text)});
    assert(!replay0.accepted,"stale replay unexpectedly advanced the interaction");
    let currentReceiver=botA;let currentSender=botB;
    for(let turn=1;turn<4;turn++){
      const row=(await client.query(`SELECT payload FROM outbox WHERE source_key=$1`,[`bot-interaction:${interaction.interactionId}:${turn}`])).rows[0];
      assert(row?.payload?.text,`turn ${turn} was not queued`);
      const result=await processBotInteractionTurn(client,{receiverBotId:currentReceiver,senderBotId:currentSender,text:String(row.payload.text)});
      if(turn<3)assert(result.accepted&&!result.completed,`turn ${turn} ended too early`);else assert(result.accepted&&result.completed,"turn budget did not complete the interaction");
      [currentReceiver,currentSender]=[currentSender,currentReceiver];
    }
    const interactionState=await client.query(`SELECT state,turn_count FROM bot_interactions WHERE id=$1`,[interaction.interactionId]);
    assert(interactionState.rows[0].state==="completed"&&Number(interactionState.rows[0].turn_count)===4,"interaction state or turn count is wrong");
    const turns=await client.query(`SELECT count(*)::int AS count FROM bot_interaction_turns WHERE interaction_id=$1`,[interaction.interactionId]);
    assert(Number(turns.rows[0].count)===4,"interaction replay created duplicate or missing turns");
    const forged=formatBotInteractionEnvelope(interaction.interactionId,0,botB,botA);
    const forgedResult=await processBotInteractionTurn(client,{receiverBotId:botB,senderBotId:botA,text:forged});
    assert(!forgedResult.accepted,"forged sender/receiver envelope was accepted");
    ...
    assert(JSON.stringify(firstAction)===JSON.stringify(replayAction),"canonical replay did not return the stored result");
    ...
    assert(Number(actionCounts.rows[0].events)===1,"canonical replay created duplicate game events");
    assert(Number(actionCounts.rows[0].stories)===1,"canonical replay created duplicate story entries");
    assert(Number(actionCounts.rows[0].xp)===3,"canonical replay granted XP more than once");

    await finalizeManagedBotRevocation(client,ownerA,botA);
    const revoked=await client.query(`SELECT enabled,revoked_at,allow_bot_interactions FROM managed_bots WHERE bot_id=$1`,[botA]);
    assert(!revoked.rows[0].enabled&&revoked.rows[0].revoked_at&&!revoked.rows[0].allow_bot_interactions,"revoke did not disable the managed bot");
    assert(!(await authorizeManagedHuman(client,{botId:botA,telegramUserId:ownerA,chatId:ownerA,chatType:"private"})),"revoked bot remained usable");
