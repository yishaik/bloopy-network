...
import {
  listOwnedManagedBots,
  setManagedBotInteractionConsent,
  upsertManagedBotAccessRule
} from "./telegram-control.js";
import {
  configureManagerWebhook,
  dispatchOutbox,
  handleManagedBotUpdate,
  handleManagerUpdate,
  managedBotCreationLink,
  migrateManagedWebhooks,
  revokeManagedBot,
  rotateManagedBotToken,
...
app.get("/health",async()=>{await db.query("SELECT 1");return {ok:true,service:"bloopy-network",version:"0.10.0"};});
...
async function enrichForPlayer(playerId:string,creatureId:string,voice:string,story:StoryCard,context:NarrativeContext){
  const profile=await loadAIProfile(playerId);const kind=providerKind(profile);let skipReason:string|undefined;
  if(kind!=="none"){
...
app.get("/auth/openrouter/callback",{logLevel:"silent",config:{rateLimit:{max:30,timeWindow:"10 minutes"}}},async(request,reply)=>{
  ...
  try{
    ...
    const exchange=await exchangeOpenRouterCode(query.data.code,claim.verifier);const keyInfo=await inspectOpenRouterKey(exchange.key);
    await withTransaction((client)=>completeOpenRouterConnection(client,claim as NonNullable<typeof claim>,exchange,keyInfo));
    destination.searchParams.set("openrouter","connected");return reply.redirect(destination.toString());
  }catch(error){
    ...
    destination.searchParams.set("openrouter","error");destination.searchParams.set("reason",claim?"connection_failed":"expired_state");return reply.redirect(destination.toString());
  }
});

app.get("/api/bootstrap",async(request)=>{
  const initData=initDataFrom(request.headers);const user=await resolveRequestUser(initData);const {player}=await withTransaction((client)=>bootstrapPlayer(client,user));
  return withTransaction(async(client)=>{
    let dashboard=await getDashboard(client,player.id);await recordPlayerActivity(client,player.id,dashboard.creature.id);
    const completed=dashboard.onboarding.status==="complete";let encounter=null;const startParam=initData?parseUnsafeStartParam(initData):null;
    if(startParam?.startsWith("meet_")&&completed){encounter=await recordEncounter(client,player.id,{id:dashboard.creature.id,name:dashboard.creature.name,slug:dashboard.creature.slug},startParam.slice(5).toLowerCase());if(encounter)dashboard=await getDashboard(client,player.id);}
    const local=await localDateForPlayer(client,player.id);const storyArc=completed?await ensureActiveStoryArc(client,player.id,dashboard.creature.id):null;
    const dailyReturn=completed?await ensureDailyReturnForDate(client,player.id,dashboard.creature.id,local.date):null;if(dailyReturn)await markDailyReturnOpened(client,player.id,dashboard.creature.id,dailyReturn.id);
    const [inventory,memories,personalityChange,profile,notifications,openrouter]=await Promise.all([
      ...
      client.query("SELECT 1 FROM ai_profiles WHERE player_id=$1 AND enabled=true AND connection_status='active'",[player.id]),getNotificationPreferences(client,player.id),getOpenRouterConnection(client,player.id)
    ]);
...
app.post("/api/admin/bots/converse",{config:{rateLimit:{max:30,timeWindow:"1 minute"}}},async(request,reply)=>{if(!config.ADMIN_API_KEY||!secureEquals(request.headers["x-admin-key"],config.ADMIN_API_KEY))return reply.code(401).send({error:"unauthorized"});const body=z.object({sourceBotId:z.number().int(),targetUsername:z.string().min(5)}).parse(request.body);return {interactionId:await withTransaction((client)=>startBotConversation(client,body.sourceBotId,body.targetUsername))};});
app.get("/api/admin/metrics",{config:{rateLimit:{max:30,timeWindow:"1 minute"}}},async(request,reply)=>{if(!config.ADMIN_API_KEY||!secureEquals(request.headers["x-admin-key"],config.ADMIN_API_KEY))return reply.code(401).send({error:"unauthorized"});const [outbox,workerLag,ai,events,security,interactions]=await Promise.all([
  db.query("SELECT status,count(*)::int AS count FROM outbox GROUP BY status"),db.query("SELECT count(*)::int AS due_pending,COALESCE(EXTRACT(EPOCH FROM (now()-min(due_at)))::int,0) AS oldest_due_seconds FROM world_events WHERE status='pending' AND due_at<=now()"),db.query("SELECT used_ai,count(*)::int AS count,COALESCE(avg(latency_ms),0)::int AS avg_latency_ms FROM ai_generation_logs WHERE created_at>now()-interval '24 hours' GROUP BY used_ai"),db.query("SELECT event_name,count(*)::int AS count FROM analytics_events WHERE created_at>now()-interval '24 hours' GROUP BY event_name ORDER BY count DESC LIMIT 30"),db.query("SELECT event_type,count(*)::int AS count FROM security_events WHERE created_at>now()-interval '24 hours' GROUP BY event_type ORDER BY count DESC"),db.query("SELECT state,count(*)::int AS count FROM bot_interactions GROUP BY state")
]);return {outbox:outbox.rows,worker:workerLag.rows[0],ai24h:ai.rows,events24h:events.rows,security24h:security.rows,botInteractions:interactions.rows,controls:{telegramIngress:config.TELEGRAM_INGRESS_ENABLED,managedFleet:config.MANAGED_BOT_FLEET_ENABLED,botToBot:config.BOT_TO_BOT_ENABLED,outbox:config.OUTBOX_ENABLED,degraded:config.DEGRADED_MODE}};});

async function processManagedUpdate(botId:number,update:TelegramUpdate){if(typeof update?.update_id!=="number")return {ok:true};await withTransaction(async(client)=>{const claimed=await client.query(`INSERT INTO telegram_updates (source,update_id) VALUES ($1,$2) ON CONFLICT DO NOTHING`,[`managed:${botId}`,update.update_id]);if(!claimed.rowCount)return;await handleManagedBotUpdate(client,botId,update);});return {ok:true};}
app.post("/telegram/manager",{config:{rateLimit:{max:300,timeWindow:"1 minute"}}},async(request,reply)=>{if(!secureEquals(request.headers["x-telegram-bot-api-secret-token"],config.TELEGRAM_WEBHOOK_SECRET))return reply.code(401).send({ok:false});if(!config.TELEGRAM_INGRESS_ENABLED)return {ok:true,paused:true};const update=request.body as TelegramUpdate;if(typeof update?.update_id!=="number")return {ok:true};await withTransaction(async(client)=>{const claimed=await client.query(`INSERT INTO telegram_updates (source,update_id) VALUES ('manager',$1) ON CONFLICT DO NOTHING`,[update.update_id]);if(!claimed.rowCount)return;await handleManagerUpdate(client,update);});return {ok:true};});
app.post("/telegram/managed/:botId",{config:{rateLimit:{max:300,timeWindow:"1 minute"}}},async(request,reply)=>{const params=z.object({botId:z.coerce.number().int()}).parse(request.params);const registry=await db.query("SELECT webhook_secret FROM managed_bots WHERE bot_id=$1 AND enabled=true AND revoked_at IS NULL",[params.botId]);if(!registry.rowCount||!secureEquals(request.headers["x-telegram-bot-api-secret-token"],registry.rows[0].webhook_secret))return reply.code(401).send({ok:false});if(!config.TELEGRAM_INGRESS_ENABLED||!config.MANAGED_BOT_FLEET_ENABLED)return {ok:true,paused:true};return processManagedUpdate(params.botId,request.body as TelegramUpdate);});
app.post("/telegram/managed/:botId/:secret",{config:{rateLimit:{max:300,timeWindow:"1 minute"}}},async(request,reply)=>{const params=z.object({botId:z.coerce.number().int(),secret:z.string().min(20)}).parse(request.params);const registry=await db.query("SELECT webhook_secret FROM managed_bots WHERE bot_id=$1 AND enabled=true AND revoked_at IS NULL",[params.botId]);if(!registry.rowCount||!secureEquals(params.secret,registry.rows[0].webhook_secret))return reply.code(401).send({ok:false});if(!config.TELEGRAM_INGRESS_ENABLED||!config.MANAGED_BOT_FLEET_ENABLED)return {ok:true,paused:true};return processManagedUpdate(params.botId,request.body as TelegramUpdate);});
...
let workerTick=0;
const workerTimer=setInterval(()=>{workerTick+=1;const runCleanup=workerTick%40===0;void withTransaction(async(client)=>{await scheduleDueDailyReturnNotifications(client);await processDueEvents(client);await dispatchOutbox(client);if(runCleanup)await cleanupProcessedWork(client);}).catch((error)=>app.log.error(error));},15_000);
workerTimer.unref();
