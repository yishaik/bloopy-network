...
export async function botCall<T>(token:string,method:string,payload:Record<string,unknown>):Promise<T>{
  ...
  if(!data.ok||data.result===undefined){
    ...
    if(typeof data.parameters?.retry_after==="number")error.retryAfter=data.parameters.retry_after;
    throw error;
  }
  return data.result;
}
...
async function registerManagedBot(client:pg.PoolClient,update:NonNullable<TelegramUpdate["managed_bot"]>):Promise<void>{
  ...
  const ownerResult=await client.query("SELECT id FROM players WHERE telegram_user_id=$1",[update.user.id]);const player=ownerResult.rows[0];
  ...
  const creatureResult=await client.query(`SELECT c.id FROM creatures c JOIN onboarding_states os ON os.creature_id=c.id AND os.status='complete' WHERE c.player_id=$1 AND c.kind='player' LIMIT 1`,[player.id]);const creature=creatureResult.rows[0];
  ...
  await client.query(`INSERT INTO managed_bots (bot_id,owner_telegram_user_id,creature_id,username,token_cipher,webhook_secret,enabled,revoked_at) VALUES ($1,$2,$3,$4,$5,$6,true,NULL) ON CONFLICT (bot_id) DO UPDATE SET owner_telegram_user_id=EXCLUDED.owner_telegram_user_id,creature_id=EXCLUDED.creature_id,username=EXCLUDED.username,token_cipher=EXCLUDED.token_cipher,webhook_secret=EXCLUDED.webhook_secret,enabled=true,revoked_at=NULL,updated_at=now()`,[update.bot.id,update.user.id,creature.id,update.bot.username??null,seal(token),webhookSecret]);
...
export async function handleManagerUpdate(client:pg.PoolClient,update:TelegramUpdate):Promise<void>{
  ...
  const message=update.message;if(!message?.from||message.from.is_bot)return;
  const {player,creature}=await bootstrapPlayer(client,message.from);const onboarding=await getOnboardingState(client,creature.id);const text=message.text??"";
  ...
  if(text.startsWith("/start")){
    if(onboarding.status!=="complete"){await reply("start-onboarding",{method:"sendMessage",text:"There is something asleep inside a cardboard nest. It needs you to decide how this story begins.",reply_markup:{inline_keyboard:[[{text:"Open the nest",web_app:{url:config.PUBLIC_BASE_URL}}]]}});return;}
    let greeting=`${creature.name} is awake and has already misplaced something important.`;const startPayload=text.split(/\s+/)[1];
    if(startPayload?.startsWith("meet_")){const met=await recordEncounter(client,player.id,{id:creature.id,name:creature.name,slug:creature.slug},startPayload.slice(5).toLowerCase());if(met)greeting=`${creature.name} and ${met.metName} are now officially acquainted.\n\n${greeting}`;}
  ...
  await client.query(`INSERT INTO memories (creature_id,source_type,summary,importance,is_private,command_key) VALUES ($1,'telegram_text',$2,0.25,true,$3) ON CONFLICT (command_key) WHERE command_key IS NOT NULL DO NOTHING`,[creature.id,text.slice(0,400),`telegram-memory:manager:${update.update_id}`]);
}

export async function handleManagedBotUpdate(client:pg.PoolClient,botId:number,update:TelegramUpdate):Promise<void>{
  const message=update.message;if(!message?.from||!message.text)return;
  await client.query(`UPDATE managed_bots SET last_webhook_at=now(),updated_at=now() WHERE bot_id=$1`,[botId]);
  if(message.from.is_bot){await processBotInteractionTurn(client,{receiverBotId:botId,senderBotId:message.from.id,text:message.text});return;}
  const registry=await authorizeManagedHuman(client,{botId,telegramUserId:message.from.id,chatId:message.chat.id,chatType:message.chat.type});if(!registry)return;
...
export async function startBotConversation(client:pg.PoolClient,sourceBotId:number,targetUsername:string):Promise<string>{return (await createBotInteraction(client,{sourceBotId,targetUsername})).interactionId;}
...
export async function rotateManagedBotToken(ownerTelegramUserId:number,botId:number):Promise<void>{
  ...
  const token=await botCall<string>(config.TELEGRAM_MANAGER_BOT_TOKEN,"replaceManagedBotToken",{user_id:botId});const webhookSecret=randomBytes(24).toString("base64url");
  await withTransaction((client)=>finalizeManagedBotRotation(client,ownerTelegramUserId,botId,seal(token),webhookSecret));await configureManagedBot(token,botId,webhookSecret);await db.query(`UPDATE managed_bots SET last_webhook_at=now(),updated_at=now() WHERE bot_id=$1`,[botId]);
}

export async function revokeManagedBot(ownerTelegramUserId:number,botId:number):Promise<void>{const bot=await withTransaction((client)=>prepareManagedBotOperation(client,ownerTelegramUserId,botId));await botCall(open(bot.tokenCipher),"deleteWebhook",{drop_pending_updates:true}).catch(()=>undefined);await withTransaction((client)=>finalizeManagedBotRevocation(client,ownerTelegramUserId,botId));}
...
export async function dispatchOutbox(client:pg.PoolClient):Promise<number>{
  ...
  for(const row of result.rows){try{let token=config.TELEGRAM_MANAGER_BOT_TOKEN;if(row.bot_id){const managed=await client.query(`SELECT token_cipher FROM managed_bots WHERE bot_id=$1 AND enabled=true AND revoked_at IS NULL`,[row.bot_id]);if(!managed.rowCount)throw new Error("managed bot is disabled or revoked");token=open(String(managed.rows[0].token_cipher));}const payload=row.payload as Record<string,unknown>&{method?:string};const {method,...body}=payload;const telegramResult=await botCall<{message_id?:number}>(token,method??"sendMessage",{chat_id:row.chat_id,...body});await client.query(`UPDATE outbox SET status='sent',attempts=attempts+1,sent_at=now(),last_error=NULL WHERE id=$1`,[row.id]);if(row.bot_id)await client.query(`UPDATE managed_bots SET last_outbound_at=now(),updated_at=now() WHERE bot_id=$1`,[row.bot_id]);if(row.daily_return_id){await client.query(`UPDATE daily_return_instances SET notification_sent_at=COALESCE(notification_sent_at,now()),updated_at=now() WHERE id=$1`,[row.daily_return_id]);if(row.player_id&&row.creature_id)await client.query(`INSERT INTO analytics_events (player_id,creature_id,event_name,properties) VALUES ($1,$2,'daily_return_notification_delivered',$3)`,[row.player_id,row.creature_id,JSON.stringify({dailyReturnId:row.daily_return_id,messageId:telegramResult.message_id??null})]);}}catch(error){const message=(error instanceof Error?error.message:"Telegram delivery failed").slice(0,500);await client.query(`UPDATE outbox SET attempts=attempts+1,status=CASE WHEN attempts+1>=7 THEN 'failed' ELSE 'pending' END,available_at=now()+(interval '1 second'*(60*POWER(2,LEAST(attempts,5))+floor(random()*30))),last_error=$2 WHERE id=$1`,[row.id,message]);console.error({error,outboxId:row.id},"outbox delivery failed");}}
