...
// Unlike the other suites this one commits, because it exercises the worker across connections.
// Every key it writes is therefore scoped to a single run, and cleaned up at the end.
const RUN=randomUUID().slice(0,8);
const UPDATE_BASE=1_000_000_000+Math.floor(Math.random()*1_000_000_000);
const key=(name:string)=>`runtime-${name}-${RUN}`;
async function insertOutbox(name:string){const sourceKey=key(name);const result=await db.query(`INSERT INTO outbox (chat_id,payload,source_key) VALUES ('123',$1,$2) RETURNING id`,[JSON.stringify({method:"sendMessage",text:sourceKey}),sourceKey]);return String(result.rows[0].id)}
...
async function main(){
  ...
  let smokePlayerId:string|null=null;
  try{
    ...
    smokePlayerId=String(player.id);
    ...
    // The batch assertions below count rows claimed process-wide, so this suite needs an idle queue.
    // Say so up front instead of failing later with a message that points at the wrong thing.
    const idle=await db.query(`SELECT (SELECT count(*)::int FROM outbox WHERE status IN ('pending','retryable','sending')) AS outbox,(SELECT count(*)::int FROM telegram_updates WHERE status IN ('received','retryable','processing')) AS updates`);
    assert(Number(idle.rows[0].outbox)===0&&Number(idle.rows[0].updates)===0,
      `this suite needs an idle queue but found ${idle.rows[0].outbox} pending outbox row(s) and ${idle.rows[0].updates} unprocessed update(s); run it against a fresh database`);

    const update:TelegramUpdate={update_id:UPDATE_BASE,message:{message_id:1,text:"hello",chat:{id:telegramId,type:"private"},from:{id:telegramId,is_bot:false,first_name:"Runtime",language_code:"en"}}};
    ...
    const successId=await insertOutbox("success");await processOutboxBatch();let delivery=await statusFor(successId);
    ...
    const retryId=await insertOutbox("retry");await processOutboxBatch();delivery=await statusFor(retryId);
    ...
    const deadId=await insertOutbox("dead");await processOutboxBatch();delivery=await statusFor(deadId);
    ...
    const uncertainId=await insertOutbox("uncertain");await processOutboxBatch();delivery=await statusFor(uncertainId);
    ...
    const expired=(await db.query(`INSERT INTO outbox (chat_id,payload,source_key,status,attempts,claim_token,claimed_at,lease_expires_at) VALUES ('123',$1,$2,'sending',1,gen_random_uuid(),now()-interval '2 minutes',now()-interval '1 minute') RETURNING id`,[JSON.stringify({method:"sendMessage",text:"expired"}),key("expired")])).rows[0];
    const staleUpdate:TelegramUpdate={update_id:UPDATE_BASE+1,message:{message_id:2,text:"sleep",chat:{id:telegramId,type:"private"},from:{id:telegramId,is_bot:false,first_name:"Runtime"}}};
    ...
    const pausedId=await insertOutbox("paused");assert(await processOutboxBatch()===0,"paused outbox still claimed work");delivery=await statusFor(pausedId);assert(delivery.status==="pending","paused outbox changed the row");
    ...
    const readyClient=await db.connect();try{await readyClient.query("BEGIN");const ready=await readinessSnapshot(readyClient);assert(ready.migrationsReady,`readiness reported missing migrations: ${ready.missingMigrations.join(", ")}`);await readyClient.query("ROLLBACK");}finally{readyClient.release();}
    console.log("Telegram delivery runtime database smoke test passed");
  }catch(error){await client.query("ROLLBACK").catch(()=>undefined);throw error}
  finally{
    globalThis.fetch=originalFetch;
    // Committed rows would otherwise collide with the next run's unique keys.
    await db.query(`DELETE FROM outbox WHERE source_key LIKE $1 OR source_key LIKE $2`,[`%-${RUN}`,`telegram-reply:manager:${UPDATE_BASE}:%`]).catch(()=>undefined);
    await db.query(`DELETE FROM telegram_updates WHERE source='manager' AND update_id = ANY($1::bigint[])`,[[UPDATE_BASE,UPDATE_BASE+1]]).catch(()=>undefined);
    await db.query(`DELETE FROM game_events WHERE command_key=$1`,[`telegram:manager:${UPDATE_BASE}`]).catch(()=>undefined);
    // Cascades the creature and everything scoped to it.
    if(smokePlayerId)await db.query(`DELETE FROM players WHERE id=$1`,[smokePlayerId]).catch(()=>undefined);
    client.release();
    await db.end();
  }
}
